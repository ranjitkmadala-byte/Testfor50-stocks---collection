...
# ============================================================
# UPSTOX MONEY-FLOW F&O MARKET ENGINE v3.6 — TOP 50 @ 09:20 IST
# ============================================================
...
def next_weekday(d):
    """Return d if Monday-Friday, otherwise the next Monday-Friday date."""
    while d.weekday() >= 5:
        d += timedelta(days=1)
    return d
...
def wait_for_valid_session_start():
    """
    Prevent an overnight Railway restart from immediately rebuilding today's
    already-finished session. Before 09:20 wait for today's freeze time.
    After market close, wait for the next weekday's 09:20 freeze time.
    """
    while True:
        now_ist = current_ist()

        if now_ist.weekday() >= 5:
            target_date = next_weekday(now_ist.date() + timedelta(days=1))
            target = datetime.combine(target_date, MONEY_FLOW_FREEZE_TIME, tzinfo=IST)
        elif now_ist.time() < MONEY_FLOW_FREEZE_TIME:
            target = datetime.combine(now_ist.date(), MONEY_FLOW_FREEZE_TIME, tzinfo=IST)
        ...
        print(
            f"Outside active startup window. Waiting until "
...
def on_open():
    print("\n" + "=" * 100)
    print("CONNECTED TO UPSTOX | MONEY-FLOW MARKET ENGINE v3.6 TOP50 @ 09:20")
    print("=" * 100)
